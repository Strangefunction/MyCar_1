...
// 角度规整与连续性处理（最终版）
// - Pitch: 限制在 [-90, +90]
// - Roll/Yaw: 规范到 [-180, +180]，并调整为与上次显示最接近的等价角，保持连续性
...
static float adjust_angle_nearest(float cur, float last_display)
{
    // 先将 cur 规范到 [-180,180]
    cur = normalize_180(cur);
    // 计算应加减的 360 倍数，使 cur 最接近 last_display
    float delta = (cur - last_display) / 360.0f;
    float n = roundf(delta);
    float adj = cur - n * 360.0f;
    adj = normalize_180(adj);
    return adj;
}
...
// 向下放置，尝试绕 X 轴旋转 180°
// 如果数据不对，可以按顺序尝试其他配置：
// 配置 1（X轴180°）: {1, 0, 0, 0, -1, 0, 0, 0, -1}  <- 当前
// 配置 2（Y轴180°）: {-1, 0, 0, 0, 1, 0, 0, 0, -1}
// 配置 3（Z轴180°）: {-1, 0, 0, 0, -1, 0, 0, 0, 1}
// 配置 4（完全反向）: {-1, 0, 0, 0, -1, 0, 0, 0, -1}
...
u8 mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    u8 retry = 0;
    // 用于处理角度连续性的静态变量
    static float last_pitch = 0.0f, last_roll = 0.0f, last_yaw = 0.0f;

    // 添加重试机制：如果 FIFO 中没有足够数据，等待后重试
    for(retry = 0; retry < 5; retry++) {
        if (dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more) == 0)
            break;  // 成功读取
        delay_ms(5);  // 等待 5ms，让 DMP 产生更多数据
    }
	...
	**/
    if (sensors & INV_WXYZ_QUAT)
    {
        ...
        *yaw = atan2(2 * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3) * 57.3; //yaw
        ...
        // 最终输出：Pitch 限制，Roll/Yaw 规范化并保持连续性
        *pitch = clamp_pitch(*pitch);
        *roll = adjust_angle_nearest(*roll, last_roll);
        *yaw = adjust_angle_nearest(*yaw, last_yaw);
