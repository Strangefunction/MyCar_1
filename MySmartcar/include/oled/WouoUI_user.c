...
typedef struct {
    ...
    uint16_t TIME_ACC;       
    uint16_t ACC;        
    uint16_t DETECTION_LOCKS;         
} ControlParams;

// 全局控制参数
ControlParams g_control_params = {
    .IO_UNLOCK = 0,      
    .HEIGHT = 100,       
    .PITCH = 75,        
    .ROLL = 75,
...
Icon main_icon_array[MAIN_PAGE_NUM] =
{
    ...
    [6] = {0xFC, 0xFE, 0xFF, 0x3F, 0x1F, 0x0F, 0x07, 0x03, 0x03, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
           0x01, 0x01, 0x01, 0x01, 0x01, 0x03, 0x07, 0x07, 0x0F, 0x1F, 0x3F, 0xFF, 0xFE, 0xFC, 0xFF, 0x01,
           0x00, 0x00, 0x00, 0x00, 0xFC, 0xFC, 0xFC, 0xFC, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFC, 0xFC, 0xFC,
           0x00, 0x00, 0xFC, 0xFC, 0xFC, 0xFC, 0x00, 0x00, 0x00, 0x00, 0x03, 0xFF, 0xFF, 0xF0, 0xC0, 0x00,
           0x00, 0x00, 0x03, 0x07, 0x0F, 0x1F, 0x3E, 0x3C, 0x3C, 0x3C, 0x1E, 0x1F, 0x0F, 0x03, 0x00, 0x00,
           0x1F, 0x3F, 0x3F, 0x1F, 0x00, 0x00, 0x00, 0xC0, 0xF0, 0xFF, 0xCF, 0xDF, 0xFF, 0xFF, 0xFE, 0xFC,
           0xF8, 0xF8, 0xF0, 0xF0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xF0, 0xF0,
           0xF8, 0xF8, 0xFC, 0xFE, 0xFF, 0xFF, 0xDF, 0xCF} // Control图标（复用Setting）
};

// 新增：控制参数页面的选项数组（匹配新结构体字段）
Option control_option_array[] = {
    {.text = (char *)"- Control Params"}, // 第0项：说明标签
    {.text = (char *)"# IO_UNLOCK", .val = 0, .decimalNum = DecimalNum_0},    // IO解禁
    {.text = (char *)"# HEIGHT", .val = 0, .decimalNum = DecimalNum_0},       // 高度
    {.text = (char *)"# PITCH", .val = 0, .decimalNum = DecimalNum_0},        // 俯仰角
    {.text = (char *)"# ROLL", .val = 50, .decimalNum = DecimalNum_0},        // 横滚角
    {.text = (char *)"# SPEED", .val = 10, .decimalNum = DecimalNum_0},       // 速度
    {.text = (char *)"# TIME", .val = 5, .decimalNum = DecimalNum_0},         // 角度时间
    {.text = (char *)"# TIME_SPEED", .val = 1, .decimalNum = DecimalNum_0},   // 速度时间
    {.text = (char *)"# TIME_ACC", .val = 0, .decimalNum = DecimalNum_0},     // 加速度时间
    {.text = (char *)"# ACC", .val = 0, .decimalNum = DecimalNum_0},          // 加速度
    {.text = (char *)"# DETECTION_LOCKS", .val = 2, .decimalNum = DecimalNum_0}, // 检测锁
};
...
bool MainPage_CallBack(const Page *cur_page_addr, InputMsg msg) {
    ...
    if(msg_click == msg){ //可以依靠text识别选项
        if (!strcmp(select_item->text, "! OddFunction")) {//设置消息弹窗内容并跳转
			snprintf(msg_buf, sizeof(msg_buf), "anweiqi verygood!"); //  把时间传递进去
            WouoUI_MsgWinPageSetContent(&common_msg_page, (char*)msg_buf);     //  第一个显示时间，页面
        ...
        } else if(!strcmp(select_item->text, "+ Control")){ // 新增：跳转到控制参数页面
            WouoUI_JumpToPage((PageAddr)cur_page_addr, &control_page);
        }
    }
    return false;
}

// 修正：控制参数页面的回调函数（匹配新选项和结构体）
bool ControlPage_CallBack(const Page *cur_page_addr, InputMsg msg) {
    Option* select_item = WouoUI_ListTitlePageGetSelectOpt(cur_page_addr);
    if(msg_click == msg){ 
        switch (select_item->order)
        {
            case 0: // 说明标签，弹出提示
                WouoUI_MsgWinPageSetContent(&common_msg_page, (char*)"Control Params Page\nSet IO/HEIGHT/PITCH etc.");
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_msg_page);
                break;
                
            case 1: // IO_UNLOCK：0 ~ 1，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 1, 1);
                common_val_page.val = g_control_params.IO_UNLOCK;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 2: // HEIGHT：0 ~ 200，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 10, 300);
                common_val_page.val = g_control_params.HEIGHT;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 3: // PITCH：-90 ~ 90，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 1, 90);
                common_val_page.val = g_control_params.PITCH;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 4: // ROLL：-90 ~ 90，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 1, 90);
                common_val_page.val = g_control_params.ROLL;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 5: // SPEED：0 ~ 50，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 1, 50);
                common_val_page.val = g_control_params.SPEED;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 6: // TIME：0 ~ 100，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 100, 2000);
                common_val_page.val = g_control_params.TIME;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 7: // TIME_SPEED：0 ~ 50，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 100, 2000);
                common_val_page.val = g_control_params.TIME_SPEED;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 8: // TIME_ACC：0 ~ 50，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 100, 2000);
                common_val_page.val = g_control_params.TIME_ACC;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 9: // ACC：0 ~ 20，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 1, 100);
                common_val_page.val = g_control_params.ACC;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
                
            case 10: // DETECTION_LOCKS：0 ~ 10，步长1
                WouoUI_ValWinPageSetMinStepMax(&common_val_page, 0, 1, 10);
                common_val_page.val = g_control_params.DETECTION_LOCKS;
                WouoUI_JumpToPage((PageAddr)cur_page_addr, &common_val_page);
                break;
...
bool CommonValPage_CallBack(const Page *cur_page_addr, InputMsg msg) {
    if (msg_click == msg) {
        ...
        //  发送命令
        else if (!strcmp(common_val_page.bg_opt->text, "# IO_UNLOCK")) {
            g_control_params.IO_UNLOCK = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_IO_UNLOCK, g_control_params.IO_UNLOCK);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# HEIGHT")) {
            g_control_params.HEIGHT = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_HEIGHT, g_control_params.HEIGHT);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# PITCH")) {
            g_control_params.PITCH = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_PITCH, g_control_params.PITCH);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# ROLL")) {
            g_control_params.ROLL = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_ROLL, g_control_params.ROLL);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# SPEED")) {
            g_control_params.SPEED = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_SPEED, g_control_params.SPEED);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# TIME")) {
            g_control_params.TIME = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_TIME, g_control_params.TIME);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# TIME_SPEED")) {
            g_control_params.TIME_SPEED = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_TIME_SPEED, g_control_params.TIME_SPEED);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# TIME_ACC")) {
            g_control_params.TIME_ACC = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_TIME_ACC, g_control_params.TIME_ACC);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# ACC")) {
            g_control_params.ACC = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_ACC, g_control_params.ACC);
        } 
        else if (!strcmp(common_val_page.bg_opt->text, "# DETECTION_LOCKS")) {
            g_control_params.DETECTION_LOCKS = (uint16_t)common_val_page.val;
            //MyCmdSet(CMD_SET_DETECTION_LOCKS, g_control_params.DETECTION_LOCKS);
        }
    }
    return false;
}
...
void TestUI_Init(void) {
    ...
    // 初始化控制参数页面选项值（从全局结构体读取）
    control_option_array[1].val = g_control_params.IO_UNLOCK;
    control_option_array[2].val = g_control_params.HEIGHT;
    control_option_array[3].val = g_control_params.PITCH;
    control_option_array[4].val = g_control_params.ROLL;
    control_option_array[5].val = g_control_params.SPEED;
    control_option_array[6].val = g_control_params.TIME;
    control_option_array[7].val = g_control_params.TIME_SPEED;
    control_option_array[8].val = g_control_params.TIME_ACC;
    control_option_array[9].val = g_control_params.ACC;
    control_option_array[10].val = g_control_params.DETECTION_LOCKS;
    ...
    WouoUI_ListPageInit(&control_page, sizeof(control_option_array)/sizeof(Option), control_option_array, Setting_none, ControlPage_CallBack);
